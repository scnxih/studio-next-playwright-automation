...
def test_17_minimum_spanning_tree_in_flow_l0(page, init):
    ...
    time.sleep(0.8)
    flow.link_two_nodes_in_flow(Helper.data_locale.SAS_PROGRAM, "LINKSETIN'中文")
    flow.arrange_nodes()
    flow.run(True)
    ...
    flow.add_step_from_stepspane_to_flow(step_path)
    flow.link_two_nodes_in_flow("LINKSETIN'中文", Helper.data_locale.STEP_MINIMUM_SPANNING_TREE)
    flow.arrange_nodes()
...
def test_20_minimum_spanning_tree_in_flow_l1(page, init):
    ...
    code = """ 
        ...
        cas;
        caslib _all_ assign;
        data casuser.'LINKSETIN''中文'n;
        set AUTOLIB.'LINKSETIN''中文'n;
        run;
        """
    sas_program_pane.type_into_text_area(code)
