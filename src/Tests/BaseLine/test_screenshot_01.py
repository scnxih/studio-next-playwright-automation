...
def test_04_screenshot_top_menu_options(page, init):
    ...
    auto.screenshot_trivial_self("autoexec_code")
    auto.click_tab_log()
    time.sleep(0.5)
    auto.screenshot_trivial_self("autoexec_log")
    auto.close_dialog()

    top_menu_page.click_options(TopMenuItem.options_custom_code)
    time.sleep(1)
    cus = CustomCodeDialog(page)
    cus.screenshot_trivial_self("customcode_pre")
    cus.click_tab_postamble()
    time.sleep(0.5)
    cus.screenshot_trivial_self("customcode_post")
    cus.click_tab_option()
    time.sleep(0.5)
    cus.screenshot_trivial_self("customcode_options")
    cus.close_dialog()
