...
"""This treeview will be used in Query left table pane, expression build left pane."""


class TreeViewCommon(CommonComponent):

    def set_base_xpath(self):
        self.base_xpath += "//div[contains(@class,'sas_components-Tree-Tree_commons-tree-container')][@role='tree']"
    ...
    def icon_expand_element(self, label):
        return self.locate_xpath("//label[text()='{0}']/../../parent::div[@aria-expanded='true']/span[1]".format(label))

    def icon_collapse_element(self, label):
        return self.locate_xpath("//label[text()='{0}']/../../parent::div[@aria-expanded='false']/span[1]".format(label))
